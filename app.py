        ...
        try:
            ...
            if line.startswith("TEMP:"):
                ...
                with data_lock:
                    ...
                    data['temp']       = float(parts.get('TEMP', 0))

                    data['bpm']        = int(parts.get('BPM', 0))

                    data['motion']     = float(parts.get('MOT', 0))

                    data['finger']     = int(parts.get('FNG', 0))

                    data['spo2']       = float(parts.get('SPO2', 0))

                    data['snd']        = int(parts.get('SND', 0))
		
                    data['btemp']      = float(parts.get('BODY_TEMP', 0))
...
<link href="https://fonts.googleapis.com/css2?family=DM+Mono:wght@300;400;500&family=Sora:wght@300;400;600&display=swap%22 rel="stylesheet">
<style>
  :root {
    --bg: #0a0e14;
    --surface: #111820;
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  h1 { font-size: 1.25rem; font-weight: 600; letter-spacing: -0.02em; color: #e8f4fd; }
  ...
  .card:hover { border-color: #2a3f55; transform: translateY(-1px); }
  ...
  .card-value {
    ...
    font-weight: 500; color: var(--card-color, var(--accent));
    letter-spacing: -0.03em; line-height: 1; transition: color 0.3s;
  }
 
  .card-unit { font-size: 0.85rem; font-weight: 300; color: var(--muted); margin-left: 3px; }
  .card-status { margin-top: 0.6rem; font-size: 0.7rem; font-family: var(--font-mono); color: var(--muted); }
...
<div class="card-label">Room temp</div>
...
<div class="card" id="card-btemp" style="--card-color: #ff8a65">   <div class="card-label">Body temp</div>   <div class="card-value" id="val-btemp">--<span class="card-unit">°C</span></div>   <div class="card-status" id="badge-btemp"></div> </div>
...
<div class="card-status" id="badge-snd" style="margin-top:0.5rem"></div>
  ...
  <div class="footer">
<span>Nucleo → Serial → Browser</span>
...
async function refresh() {
  try {
    ...
    document.getElementById('badge-temp').innerHTML = t < 36.0 ? badge('warn','Low') : t > 37.5 ? badge('alert','High') : badge('ok','Normal');
    flash('card-temp');
 
    const bpm = parseInt(d.bpm);
    document.getElementById('val-bpm').innerHTML = (bpm > 0 ? bpm : '--') + '<span class="card-unit">bpm</span>';
    document.getElementById('badge-bpm').innerHTML = bpm === 0 ? badge('off','No reading') : bpm < 60 || bpm > 100 ? badge('warn','Check') : badge('ok','Normal');
    flash('card-bpm');
 
    const spo2 = parseFloat(d.spo2);
    document.getElementById('val-spo2').innerHTML = (spo2 > 0 ? spo2.toFixed(1) : '--') + '<span class="card-unit">%</span>';
    document.getElementById('badge-spo2').innerHTML = spo2 === 0 ? badge('off','No reading') : spo2 < 95 ? badge('alert','Low') : badge('ok','Normal');
    flash('card-spo2');

const bt = parseFloat(d.btemp); document.getElementById('val-btemp').innerHTML = (bt > 0 ? bt.toFixed(1) : '--') + '<span class="card-unit">°C</span>'; document.getElementById('badge-btemp').innerHTML = bt === 0 ? badge('off','No reading') : bt < 36.0 ? badge('warn','Low') : bt > 37.5 ? badge('alert','High') : badge('ok','Normal'); flash('card-btemp');
 
    const mot = parseFloat(d.motion);
    document.getElementById('val-motion').innerHTML = mot.toFixed(2) + '<span class="card-unit">g-dev</span>';
    document.getElementById('badge-motion').innerHTML = mot > 15.0 ? badge('warn','Active') : badge('ok','Calm');
    flash('card-motion');
    ...
    document.getElementById('val-snd').innerHTML = snd + '<span class="card-unit">%</span>';
    const sndPct = Math.min(100, (snd / 4095) * 100).toFixed(1);
    document.getElementById('sound-bar').style.width = sndPct + '%';
    document.getElementById('badge-snd').innerHTML = snd > 2000 ? badge('alert','Crying!') : snd > 500 ? badge('warn','Noise') : badge('ok','Quiet');
    flash('card-snd');
